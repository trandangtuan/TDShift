...
import { Check, CheckCheck, Edit3, Hash, MoreHorizontal, Paperclip, Plus, Search, Send, Settings2, Trash2, UserMinus, Users, X } from "lucide-react";
...
export default function DiscussChat({ api, user }: DiscussChatProps) {
  ...
  async function loadUsers() {
    ...
    setUsers(data.records.map((record) => ({ id: Number(record.id), name: String(record.name ?? record.login ?? "User"), login: record.login as string | undefined, email: record.email as string | undefined, phone: record.phone as string | undefined })));
  }
  ...
  async function loadChannel(channelId: number) {
    ...
    setMessages(messageData.records.map((record) => ({ id: Number(record.id), channelId, authorId: relationId(record.author_id) ?? 0, authorName: relationName(record.author_id) ?? "User", body: String(record.body ?? ""), sentAt: String(record.sent_at ?? "") })));
  ...
  async function deleteChannel() {
    ...
    if (!window.confirm(`Xóa channel "${selectedChannel.name}" và toàn bộ tin nhắn?`)) return;
