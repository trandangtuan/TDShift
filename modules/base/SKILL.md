...
- `ir.attachment` stores file metadata in the database and stores uploaded file bytes in MinIO. `datas` is accepted as base64 input, uploaded to MinIO, then cleared from metadata. URL attachments may still use `storage = url`.
...
- Attachments: go to Settings > Technical > Attachments, choose a local file in Upload File, and the UI uploads it through `/api/attachments/upload` as multipart data. The server streams it to MinIO, creates `ir.attachment`, and stores `bucket`, `object_name`, `file_size`, and `checksum`.
...
- Large files must not be sent as base64 JSON. Use multipart upload and stream to MinIO.
