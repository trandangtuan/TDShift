...
export const websiteSaleRoutes: ModuleRoute[] = [{
  register({ app, db }) {
    app.get("/api/website-sale/products", async () => ({ products: publishedProduct(db) }));
    app.get("/api/website-sale/products/:slug", async (request: any, reply: any) => {
      const product = publishedProduct(db).find((candidate) => candidate.slug === request.params.slug);
...
function publishedProduct(db: any) {
